...
def main():
    global args
    parser = argparse.ArgumentParser('list missing latin, greek, and cyrillic composite glyphs')
    parser.add_argument('filename')
    args = parser.parse_args()
    font = fontforge.open(args.filename)
    for list in [CHARBLOCKS]:
        for block in list:
            found_in_block = False
            for codepoint in range(block[0], block[1]):
                if sys.stderr.isatty() and codepoint % 16 == 0:
                    sys.stderr.write("\rU+%04X\x1b[K" % codepoint)
                glyphname = fontforge.nameFromUnicode(codepoint)
                if glyphname in font:
                    continue
                new_glyph = font.createChar(codepoint)
                new_glyph.build()
                if len(new_glyph.references):
                    if not found_in_block:
                        print("# %s" % block[2])
                        found_in_block = True
                    print("%-8s  %s" % (u(codepoint), unicodedata.name(chr(codepoint))))
            if not found_in_block:
                print("# (none) %s" % block[2])
    if sys.stderr.isatty() and codepoint % 16 == 0:
        sys.stderr.write("\r\x1b[K" % codepoint)
